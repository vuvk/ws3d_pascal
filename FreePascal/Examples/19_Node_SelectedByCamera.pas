...
'' Адаптировал Vuvk
'' ----------------------------------------------------------------------------
'' Пример 19 : Выбор определённого нода при движении камеры
'' В примере показана возможность  выбора определённого нода на сцене. Это делается
'' через исходящий из центра камеры луч, достигающий нужного нода.
'' ----------------------------------------------------------------------------
}
...
begin
  ...
  for i := 0 to TEST_NODES - 1 do
    begin
      TestNode[i] := wNodeCreateCube(10, wCOLOR4s_WHITE);
  ...
  OurCamera := wFpsCameraCreate(100, 0.1, PwKeyMapDefault, Length(wKeyMapDefault));
