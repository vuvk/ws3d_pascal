 ...
 for i:=entityIndex.x  to entityIndex.y do
    ...
    if entityName='info_player_start' then
       begin
   	 ...
   	 BillBoard:=wBillboardCreateText(position,bSize,wFontGetDefault(),'PLAYER START',bTopColor,bBottomColor);
    end;
	...
	angle:=wBspGetVarGroupValueAsFloat(varGroup,'angle',parsePos);
       ...
       BillBoard:=wBillboardCreateText(position,bSize,wFontGetDefault(),'PLAYER DEATHMATCH No '+FormatFloat('0',count),bTopColor,bBottomColor);
